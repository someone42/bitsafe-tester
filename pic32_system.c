...
/** Current LED that is on: 0 = red, 1 = green, 2 = blue. */
static int led_sequence_counter;
...
/** Turn off current LED and light up next LED in sequence. */
static void toggleLEDSequence(void)
{
	if (led_sequence_counter == 0)
	{
		// Red LED on.
		PORTDCLR = 0x04;
		PORTDSET = 0x11;
	}
	else if (led_sequence_counter == 1)
	{
		// Green LED on.
		PORTDCLR = 0x10;
		PORTDSET = 0x05;
	}
	else if (led_sequence_counter == 2)
	{
		// Blue LED on.
		PORTDCLR = 0x15;
	}

	led_sequence_counter++;
	if (led_sequence_counter == 3)
	{
		led_sequence_counter = 0;
	}
}
